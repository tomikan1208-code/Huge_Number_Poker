 ...
 *
 * 定数は**目標値に自動で当てたものではない**。手で決めて、決めた理由を
 * その定数のコメントに書いてある。根拠のあるもの（九九1回 ≈ 1秒、書字速度）と
 * 判断で置いたものを混ぜないこと。
 * いじったら必ず `node tools/calibrate-ai.js` を通す（要件の検査＋記録との差分）。
 *
 * ============================================================
...
const COG = {
  ...
  ADD_PER_COLUMN: 0.15,
  /**
   * 九九（1桁×1桁）1回。計算ではなく記憶からの検索。
   * 成人が「7×8」に答えるまで約1秒（Campbell & LeFevre 2001）。ここは実測値に近い。
   *
   * 単発で問われるときも筆算の途中で引くときも**同じ 0.80 を使う**。
   * 以前は「筆算は手順が流れに乗っているぶん速いはず」として途中の想起を
   * 0.439 に分けていたが、これは裏の取れていない仮定だった。
   * 九九の想起時間として根拠があるのは 1回あたり約1秒という値だけなので、
   * 場面で使い分けず1つの定数に統一する。速いという証拠が出たら分ければよい。
   */
  MUL_TABLE: 0.80,
  ...
  if (top === 1 && bottom === 1) {
    ...
    for (let i = 0; i < bottom; i++) {
      ...
      push(
        ...
        top * COG.MUL_TABLE + (top + 1) * COG.WRITE_PER_DIGIT
      );
    }
    if (bottom > 1) {
      push(
