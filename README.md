...
なお **CPU は相手を読んでいない**。相手モデルは固定で、誰が何をしたかは
見ていない。ブラフも「弱いときに一定確率で仕掛ける」であって
「相手が弱そうだから仕掛ける」ではない。
...
> `models/policy_skilled.json` はこの変更より前（リバイ付き・bb/hand 報酬）に
> 作られたもの。トーナメント特有の降り引き際は入っていないので、いずれ再学習が要る。
